...
| list_cons
  ...
  | list_nil((*void*))=>
    let val () = auxerr(loc0, d2m, 1) in res end
  )
| list_nil((*void*)) =>
  (
  case+ s2es of
...
| list_cons
  ...
  | _ => let
      val m2vs =
        auxexplst(loc0, knd, ctx, env, d2es)
      // end of [val]
    in
...
in
  the_trans3errlst_add
    (T3E_dmacro_eval0_app_mac_arity(loc0, d2m, d2as))
  // the_trans3errlst_add
end // end of [if]
) : void // end of [val]
//
...
val d2e =
  d2mac_get_def(d2m)
val m2v =
  eval0_d2exp(loc0, ctx_new, env, d2e)
//
...
eval1_d2exp
(
loc0, ctx_new, env, d2mac_get_def(d2m)
) (* end of [val] *)
//
