...
| S2Ewithout (s2e) => aux_s2exp (s2e, fvs) // taken out by [view@]
//
...
extern fun aux_s2cst : aux_type (s2cst)
extern fun aux_s2var : aux_type (s2var)
extern fun aux_s2Var : aux_type (s2Var)

extern fun aux_s2exp : aux_type (s2exp)
extern fun aux_s2explst : aux_type (s2explst)
extern fun aux_s2explstlst : aux_type (s2explstlst)

extern fun aux_s2eff : aux_type (s2eff)

extern fun aux_s2lab : aux_type (s2lab)
extern fun aux_s2lablst : aux_type (s2lablst)

extern fun aux_labs2exp : aux_type (labs2exp)
extern fun aux_labs2explst : aux_type (labs2explst)
...
macdef
f_s2cst (x) =
  aux_s2cst (s2V0, ,(x), ans, s2cs, s2vs, s2Vs)
macdef
f_s2var (x) =
  aux_s2var (s2V0, ,(x), ans, s2cs, s2vs, s2Vs)
macdef
f_s2Var (x) =
  aux_s2Var (s2V0, ,(x), ans, s2cs, s2vs, s2Vs)
macdef
f_s2exp (x) =
  aux_s2exp (s2V0, ,(x), ans, s2cs, s2vs, s2Vs)
macdef
f_s2explst (x) =
  aux_s2explst (s2V0, ,(x), ans, s2cs, s2vs, s2Vs)
macdef
f_s2explstlst (x) =
  aux_s2explstlst (s2V0, ,(x), ans, s2cs, s2vs, s2Vs)
macdef
f_s2eff (x) =
  aux_s2eff (s2V0, ,(x), ans, s2cs, s2vs, s2Vs)
macdef
f_s2lablst (x) =
  aux_s2lablst (s2V0, ,(x), ans, s2cs, s2vs, s2Vs)
macdef
f_labs2explst (x) =
  aux_labs2explst (s2V0, ,(x), ans, s2cs, s2vs, s2Vs)
macdef
f_wths2explst (x) =
  aux_wths2explst (s2V0, ,(x), ans, s2cs, s2vs, s2Vs)
//
(*
val () = println! ("aux_s2exp: s2V0 = ", s2V0)
val () = println! ("aux_s2exp: s2e = ", s2e)
val () = println! ("aux_s2exp: ans = ", ans)
val () = println! ("aux_s2exp: s2cs = ", s2cs)
...
| S2EFFadd (s2fe1, s2fe2) => let
    val () =
      aux_s2eff (s2V0, s2fe1, ans, s2cs, s2vs, s2Vs)
    val () =
      aux_s2eff (s2V0, s2fe2, ans, s2cs, s2vs, s2Vs)
  in
    // nothing
  end // end of [S2EFFadd]
//
...
| WTHS2EXPLSTcons_invar
    ...
    // end of [val]
  in
    aux_wths2explst (s2V0, ws2es, ans, s2cs, s2vs, s2Vs)
  end
| WTHS2EXPLSTcons_trans
    (_, s2e, ws2es) => let
    val () =
