...
#dynload "./atscc2erl_emit.dats"
#dynload "./atscc2erl_emit2.dats"
...
| list_nil () => let
  ...
  in
    if wait0 then (
      if state.ncomarg = 0
        then atscc2erl_usage ("atscc2erl")
        else atscc2erl_fileref (state, stdin_ref)
    ) (* end of [if] *)
  end // end of [list_nil]
//
| list_cons
...
| _ when
  ...
  in
    ...
    | COMARGkey (_, fname) => let
        ...
        val () = atscc2erl_basename (state, fname(*input*))
      in
