...
implement{a}
SelectionSort__cmp (x1, x2) = gcompare_ref (x1, x2)
...
fun findmin
  ...
  pf1: array_v (a, l1, n1)
, pf2: array_v (a, l2, n2) // the first element of this segment is the current min
, pf3: array_v (a, l3, n3)
| p1: ptr l1, p2: ptr l2, p3: ptr l3
, n3: size_t n3
) :<!wrt> (array_v (a, l1, n1+n2+n3) | void) =
  if n3 > 0 then let
    prval (pf21, pf22) = array_v_uncons (pf2)
    prval (pf31, pf32) = array_v_uncons (pf3)
    val sgn = SelectionSort__cmp (!p2, !p3)
  ...
  in
    ...
    in
      findmin (pf1, pf2, pf32 | p1, p3, ptr1_succ<a> (p3), pred(n3))
    end else let
      prval pf2 = array_v_extend (pf2, pf31)
    in
      findmin (pf1, pf2, pf32 | p1, p2, ptr1_succ<a> (p3), pred(n3))
    end // end of [if]
  end else let
...
fun loop
  ...
  if n >= 2 then let
    ...
    val p1 = ptr1_succ<a> (p) and n1 = pred(n)
    val (pfmin | ()) = findmin (
